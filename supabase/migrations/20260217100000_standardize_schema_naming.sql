...
ALTER TABLE IF EXISTS public.bids
  RENAME COLUMN "is_proxy" TO is_proxy;
ALTER TABLE IF EXISTS public.bids
  RENAME COLUMN "max_bid" TO max_bid;
...
CREATE POLICY "Users can manage own watchlist" ON public.watchlists FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
