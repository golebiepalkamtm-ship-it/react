...
export const ChampionImageDiagnostics = () => {
  ...
  useEffect(() => {
    const runDiagnostics = async () => {
      try {
        ...
        const promises = manifest.champions.map(async (item): Promise<DiagnosticResult> => {
  ...
  return (
    <div className="diagnostics-overlay">
      ...
      <ul>
        {results.map(result => (
          <li key={result.id} className={`diagnostics-item ${result.status === 'SUCCESS' ? 'success' : 'fail'}`}>
            <strong>{result.status}</strong> - (HTTP {result.statusCode}) - <a href={result.imageUrl} target="_blank" rel="noopener noreferrer" className="diagnostics-link">{result.imageUrl}</a>
          </li>
        ))}
      </ul>
    </div>
