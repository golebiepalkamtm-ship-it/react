...
interface AdminUsersTableProps {
    ...
    onAction: (userId: string, action: 'ban' | 'unban' | 'delete' | 'verify') => void;
    onAdd: () => void;
}
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <p className="text-white/60 text-sm">Znaleziono {users.length} użytkowników</p>
                <Button
                    onClick={onAdd}
                            ...
                            >
                                ...
                                <td className="py-4 px-6">
                                    <div className="flex items-center justify-end gap-2">
                                        <button className="p-2 rounded-lg bg-white/5 hover:bg-blue-500/20 transition-colors" onClick={() => onEdit(user)}>
                                            <Edit className="w-4 h-4 text-blue-400" />
                                        </button>
                                        <button className="p-2 rounded-lg bg-white/5 hover:bg-green-500/20 transition-colors" onClick={() => onAction(user.id, 'verify')}>
                                            <UserCheck className="w-4 h-4 text-green-400" />
                                        </button>
                                        {user.role === 'BANNED' ? (
                                            <button className="p-2 rounded-lg bg-white/5 hover:bg-green-500/20 transition-colors" onClick={() => onAction(user.id, 'unban')}>
                                                <CheckCircle className="w-4 h-4 text-green-400" />
                                            </button>
                                        ) : (
                                            <button className="p-2 rounded-lg bg-white/5 hover:bg-orange-500/20 transition-colors" onClick={() => onAction(user.id, 'ban')}>
                                                <Ban className="w-4 h-4 text-orange-400" />
                                            </button>
                                        )}
                                        <button className="p-2 rounded-lg bg-white/5 hover:bg-red-500/20 transition-colors" onClick={() => onAction(user.id, 'delete')}>
