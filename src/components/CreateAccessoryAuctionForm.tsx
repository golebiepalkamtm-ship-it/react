...
import { X, AlertCircle, Loader2, Check, Sparkles, Camera, Video, Wrench, Package } from 'lucide-react';
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setFeedbackModal({
        ...
        type: 'success',
        title: 'Sukces!',
        message: 'Aukcja akcesoriów została utworzona pomyślnie.'
      });
    } catch (err) {
      console.error(err);
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          <motion.div 
            ...
            animate={{ 
              boxShadow: ['0 10px 30px rgba(245,158,11,0.3)', '0 10px 50px rgba(245,158,11,0.5)', '0 10px 30px rgba(245,158,11,0.3)']
            }}
          ...
          <div>
            <h3 className="text-xl font-bold text-white">Nowa aukcja akcesoriów</h3>
            <p className="text-sm text-white/60">Sprzęt i wyposażenie gołębnika</p>
          </div>
        </div>

        <InputField
          label="Nazwa akcesorium"
          name="title"
          value={formData.title}
          onChange={handleChange}
          placeholder="np. Transporter gołębi profesjonalny"
          required
          delay={0.15}
        />
        ...
        <InputField
          ...
          onChange={handleChange}
          placeholder="Opisz akcesorium, jego stan, parametry..."
          type="textarea"
          required
          delay={0.2}
        />
      ...
      >
        ...
        <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="flex-1 md:flex-[2]">
          <Button 
            type="submit" 
            disabled={loading} 
            className="w-full py-6 rounded-xl bg-gradient-to-r from-amber-500 via-orange-500 to-amber-500 text-white font-bold shadow-lg shadow-amber-500/30 hover:shadow-xl hover:shadow-amber-500/40 transition-all disabled:opacity-50"
          >
            {loading ? (
              <motion.div 
