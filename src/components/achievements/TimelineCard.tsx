...
const TimelineCard = ({ event, index, isActive }: TimelineCardProps) => {
  ...
  // Spring physics removed for performance (handled by Lenis)
  const y = useTransform(scrollYProgress, [0, 0.5, 1], [120, 0, -80]);

  // Adjusted opacity to show earlier (0.01 instead of 0.12) - fixes dark first cards
  // DLA PIERWSZYCH KART (index < 3): Zawsze w pełni widoczne na starcie
  const scrollOpacity = useTransform(
    scrollYProgress,
    [0, 0.01, 0.9, 1], // Show much earlier (0.01) and stay visible longer (0.9)
    [0, 1, 1, 0],
  );

  const cardOpacity = index < 3 ? 1 : scrollOpacity;
  ...
  const getRank = (text: string): "gold" | "silver" | "bronze" | null => {
    ...
    if (
      lower.includes("mistrz") &&
      !lower.includes("wicemistrz") &&
      !lower.includes("v-ce")
    )
      return "gold";
  ...
  return (
    <div
      ref={cardRef}
      className="timeline-card-wrapper relative mb-32 md:mb-44 will-change-transform"
    >
      <motion.div style={{ y, opacity: cardOpacity }} className="w-full">
        <div
        ...
        >
          ...
          >
            {/* Ambient Glow Layer — Tight Rim Light (Exactly matches card size) */}
            <motion.div
            ...
            <motion.div
              ...
              // Unified rectangular reveal — consistent size for all cards
              initial={{
            ...
            >
              ...
              <div className="awwwards-card-body p-8 md:p-10">
                ...
                <motion.div className="absolute bottom-0 left-0 right-0 h-[1px] overflow-hidden">
                  <motion.div
