        ...
        <div className="space-y-4">
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              <div
                className={`p-3 rounded-xl bg-gradient-to-br ${categoryConfig.color} shadow-lg shadow-gold/20`}
              >
          ...
          {Array.from({ length: totalSteps }, (_, i) => i + 1).map((step) => (
            <React.Fragment key={step}>
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-[10px] uppercase font-bold tracking-widest hidden md:block ${currentStep >= step ? "text-gold" : "text-white/20"}`}
                >
                  {category === "pigeons"
                    ? step === 1
                      ? "Podstawowe"
              ...
              {step < totalSteps && (
                <div
                  className={`w-8 h-px ${currentStep > step ? "bg-gold/40" : "bg-white/10"}`}
                />
              )}
            </React.Fragment>
          ))}
      ...
      <div className="flex gap-4 pt-2">
        {onCancel && (
          <Button
            variant="outline"
            onClick={onCancel}
            className="flex-1 py-6 border-white/10 text-white hover:bg-white/5"
          >
            Anuluj
          </Button>
        )}
