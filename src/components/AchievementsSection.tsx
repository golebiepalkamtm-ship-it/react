...
const AchievementsSection = ({ showHeader = true }: AchievementsSectionProps) => {
  ...
  const smoothProgress = useSpring(scrollYProgress, {
    stiffness: 600, // Zwiększone dla mniejszego oscylowania
    damping: 80,   // Zwiększone dla stabilności
    restDelta: 0.001,
  });

  // Perspective transforms for tunnel effect
  const perspectiveZ = useTransform(smoothProgress, [0, 1], [0, -300]); // Zmniejszone z -500
  const tunnelOpacity = useTransform(smoothProgress, [0, 0.1, 0.9, 1], [0, 1, 1, 0]);

  // Update active index based on scroll - bardziej agresywny debounce
  useEffect(() => {
  ...
  return (
    <section
      id="achievements"
      ref={containerRef}
      className="achievements-tunnel relative isolate min-h-screen overflow-x-hidden z-20"
    >
      ...
      <div className="fixed inset-0 overflow-hidden tunnel-perspective pointer-events-none z-2">
        ...
        >
          {[...Array(3)].map((_, i) => ( // Zmniejszone z 5 do 3
            <motion.div
              key={i}
              className="absolute border border-primary/10 rounded-full"
              style={{
                width: `${(i + 1) * 40}%`, // Zwiększone dla lepszej widoczności
                height: `${(i + 1) * 40}%`,
                transform: `translateZ(${i * -150}px)`, // Zmniejszone z -100px
              }}
              animate={{
                opacity: [0.05, 0.15, 0.05], // Zmniejszone opacity
                scale: [1, 1.01, 1], // Minimalne skalowanie
              }}
              transition={{
                duration: 6, // Zwiększone z 4
                repeat: Infinity,
                delay: i * 0.8, // Zwiększone opóźnienie
                ease: "easeInOut"
              }}
            />
          ))}
        ...
        {showHeader ? (
          ...
          >
            ...
            <p className="text-muted-foreground text-lg md:text-xl max-w-2xl mx-auto">
              Przewiń czas i odkryj historię sukcesów od 2001 roku
            </p>
