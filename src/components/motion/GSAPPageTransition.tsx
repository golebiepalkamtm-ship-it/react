...
const routeTransitionStyles: Record<string, TransitionStyle> = {
  // Wszystkie strony używają domyślnego stylu "reveal" wg życzenia użytkownika
};
...
export const GSAPPageTransition = ({
  ...
  const animateReveal = (tl: gsap.core.Timeline, dur: number, color: string) => {
    ...
    // Simple reveal with rotating star
    tl.set(overlayRef.current, {
      clipPath: "circle(0% at 50% 50%)",
      visibility: "visible",
      background: `radial-gradient(circle, ${color} 0%, #0a0a0f 100%)`,
    })
      .set(logoRef.current, { opacity: 0, scale: 0.8, rotation: 0 })
      .to(contentRef.current, {
        opacity: 0,
        duration: dur * 0.2,
        ease: "power2.in",
