...
const envSchema = z.object({
  ...
  VITE_SUPABASE_PUBLISHABLE_KEY: z.string().min(100, 'VITE_SUPABASE_PUBLISHABLE_KEY seems too short'),
  VITE_SUPABASE_ANON_KEY: z.string().min(100, 'VITE_SUPABASE_ANON_KEY seems too short').optional(),
...
function validateEnv(): EnvSchema {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      throw new Error('Invalid frontend environment configuration. Check console above.');
    }
    throw error;
  }
}
...
export function createFrontendConfig(): FrontendConfig {
  ...
  const wsUrl = env.VITE_WS_URL || env.VITE_API_URL.replace('/api', '').replace(/\/$/, '');
