...
class WebsocketService {
  ...
  constructor() {
    const base =
      sanitizeEnvValue(import.meta.env.VITE_WS_URL) ||
  ...
  private getReconnectDelay(): number {
    ...
    return this.reconnectAttempts < delays.length
      ? (delays[this.reconnectAttempts] ?? 30000)
      : 30000;
  }
  ...
  connect(token: string) {
    ...
    this.socket.on("connect_error", (error) => {
      ...
      const isAuthError =
        /invalid or expired token|no authentication credentials provided|origin not allowed/i.test(
          message,
        );
      if (isAuthError) {
        this.disconnect();
  ...
  onBidPlaced(
    handler: (payload: {
      ...
      newPrice: number;
      auctionId: string;
      meta?: { wasExtended?: boolean; newEndTime?: string | null };
    }) => void,
  ) {
  ...
  onViewersCount(
    handler: (payload: { auctionId: string; count: number }) => void,
  ) {
    this.socket?.on("viewers-count", handler);
  }

  offViewersCount(handler: (payload: any) => void) {
    this.socket?.off("viewers-count", handler);
  }
