                    ...
                    <div className="space-y-4">
                        {topics?.length === 0 ? (
                            <div className="text-center py-20 bg-white/5 rounded-2xl border border-white/10">
                                ...
                                <p className="text-white/40">Brak tematów w tej kategorii. Bądź pierwszym, który go utworzy!</p>
                            </div>
                        ) : (
                                    ...
                                    >
                                        <div className="flex gap-4 items-center">
                                            ...
                                            <div>
                                                <h3 className="text-xl font-bold mb-1 group-hover:text-gold transition-colors">
                                                    {topic.is_pinned && <span className="text-gold mr-2 text-sm uppercase">Pinned</span>}
                                                    {topic.title}
                                                </h3>
                                                <div className="flex items-center gap-4 text-sm text-white/40">
                                                    <span className="flex items-center gap-1">
                                                        <User size={14} />
                                                        {topic.author?.profile?.name || topic.author?.email || 'Anonim'}
                                                    </span>
                                                    <span className="flex items-center gap-1">
                                                        <Clock size={14} />
                                        ...
                                        <div className="flex items-center gap-8 pr-4">
                                            <div className="text-center">
                                                <div className="text-lg font-bold">{topic.posts_count || 0}</div>
                                                <div className="text-[10px] uppercase tracking-widest text-white/40">Odpowiedzi</div>
                                            </div>
                                            <div className="text-center">
                                                <div className="text-lg font-bold">{topic.views_count || 0}</div>
                                                <div className="text-[10px] uppercase tracking-widest text-white/40">Wyświetleń</div>
                                            </div>
