...
const AdminPage: React.FC = () => {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      const data = await apiClient.getWithToken<{ users: UserItem[] }>('/admin/users', undefined, session.access_token);
      setUsers(data.users || []);
    } catch (e) {
      logger.error(e);
    } finally {
  ...
  const fetchStats = React.useCallback(async () => {
    try {
      if (!session?.access_token) return;
      const stats = await apiClient.getWithToken('/admin/stats', undefined, session.access_token);
      setStats(stats);
    } catch (e) {
      logger.error(e);
    }
