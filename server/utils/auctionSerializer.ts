...
const toLowerEnum = (value: string | null | undefined) =>
...
export function serializePigeon(
  ...
  return {
    ...
    gender: toLowerEnum(pigeon.gender as string) as
      | "male"
      | "female"
      | undefined,
  ...
  return {
    ...
    createdAt:
      bid.createdAt instanceof Date
        ...
        : (bid.createdAt as unknown as string),
  };
}

export function serializeAuction<T extends AuctionEntity | AuctionListEntity>(
  auction: T,
  ...
  return {
    ...
    endTime:
      auction.endTime instanceof Date
        ...
        : (auction.endTime as unknown as string),
    status: toLowerEnum(auction.status) as
      | "active"
      | "ended"
      | "cancelled"
      | undefined,
...
export function serializePublicAuction<
  T extends AuctionEntity | AuctionListEntity,
>(auction: T, userId?: string) {
  return serializeAuction(auction, { isOwner: false, userId });
}

// Private serializer - pełne dane tylko dla owner/admin
export function serializePrivateAuction<
  T extends AuctionEntity | AuctionListEntity,
>(auction: T, userId?: string) {
