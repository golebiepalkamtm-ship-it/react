...
export function serializeAuction<T extends AuctionEntity | AuctionListEntity>(auction: T, options: { isOwner?: boolean; userId?: string } = {}) {
  ...
  return {
    ...
    gender: toLowerEnum((auction as any).gender) as 'male' | 'female' | 'young' | undefined,
    ...
    bids: Array.isArray(auction.bids) ? (auction.bids as any[]).map((b) => serializeBid(b as BidEntity, showContact)) : [],
