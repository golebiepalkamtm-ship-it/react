...
export const setupWebSocketEvents = (io: Server) => {
  io.use(async (socket, next) => {
    try {
      ...
      const ticket =
        socket.handshake.auth.ticket || socket.handshake.query.ticket;

      if (ticket) {
        // Ticket-based authentication (recommended)
        try {
          const ticketData = wsTicketService.validateAndConsumeTicket(
            ticket as string,
          );
          ...
          logger.info(
            `WebSocket authenticated via ticket: ${ticketData.userId}`,
          );
          return next();
        } catch (error) {
      ...
      try {
        const verificationResult = await verifyJWTTokenWithRole(
          token,
          rateLimitKey,
        );
        ...
        logger.info(
          `WebSocket authenticated via JWT: ${verificationResult.userId}`,
        );
        next();
      } catch (error) {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("join-auction", (auctionId: string) => {
      ...
      const viewersCount = io.sockets.adapter.rooms.get(roomName)?.size || 0;
      io.to(roomName).emit("viewers-count", { auctionId, count: viewersCount });
      logger.info(
        `User ${socket.data.userId} joined auction ${auctionId}, viewers: ${viewersCount}`,
      );
    });
    ...
    socket.on("leave-auction", (auctionId: string) => {
      ...
      io.to(roomName).emit("viewers-count", { auctionId, count: viewersCount });
      logger.info(
        `User ${socket.data.userId} left auction ${auctionId}, viewers: ${viewersCount}`,
      );
    });
    ...
    socket.on(
      ...
      async (data: {
        ...
        try {
          ...
          logger.info(
            `Bid placed via WS: ${amount} by ${userId} on auction ${auctionId}`,
          );
    ...
    socket.on("disconnecting", () => {
      socket.rooms.forEach((roomName) => {
        if (roomName.startsWith("auction-")) {
          const auctionId = roomName.replace("auction-", "");
          const viewersCount =
            (io.sockets.adapter.rooms.get(roomName)?.size || 1) - 1; // pomniejszamy o 1 bo socket.rooms jeszcze go zawiera
          io.to(roomName).emit("viewers-count", {
            auctionId,
            count: viewersCount,
          });
          logger.info(
            `User ${socket.data.userId} disconnected from auction ${auctionId}, viewers: ${viewersCount}`,
          );
        }
      });
    });
