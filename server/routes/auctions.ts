...
import {
  serializeAuction,
  serializePrivateAuction,
  serializePublicAuction,
  baseAuctionInclude,
...
router.get("/", async (req, res) => {
  try {
    ...
    const normalizedQuery = validationResult.success
    ...
    const normalizedSortBy =
      typeof sortBy === "string" ? sortBy.split(":")[0] : undefined;
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch auctions", code: error.code });
      return;
    }
...
async function performAuctionCreate(
  ...
  if (typeof pedigreeUrl === "string" && pedigreeUrl.trim().length > 0) {
    documentsCreate = [...(documentsCreate || []), { url: pedigreeUrl.trim() }];
  }
  ...
  const rawPigeonPayload =
    pigeon && typeof pigeon === "object"
      ? {
          ringNumber:
            typeof pigeon.ringNumber === "string"
              ? pigeon.ringNumber.trim()
              : undefined,
          eyeColor:
            typeof pigeon.eyeColor === "string" ? pigeon.eyeColor : undefined,
          featherColor:
            typeof pigeon.pigeonColor === "string"
              ? pigeon.pigeonColor
              : undefined,
          construction:
            typeof pigeon.construction === "string"
              ? pigeon.construction
              : undefined,
          vitality:
            typeof pigeon.vitality === "string" ? pigeon.vitality : undefined,
          length: typeof pigeon.length === "string" ? pigeon.length : undefined,
          endurance:
            typeof pigeon.endurance === "string" ? pigeon.endurance : undefined,
          forkStrength:
            typeof pigeon.forkStrength === "string"
              ? pigeon.forkStrength
              : undefined,
          forkAlignment:
            typeof pigeon.forkAlignment === "string"
              ? pigeon.forkAlignment
              : undefined,
          muscles:
            typeof pigeon.muscles === "string" ? pigeon.muscles : undefined,
          balance:
            typeof pigeon.balance === "string" ? pigeon.balance : undefined,
          back: typeof pigeon.back === "string" ? pigeon.back : undefined,
          purpose:
            typeof pigeon.purpose === "string" ? pigeon.purpose : undefined,
          traits: traits as any,
        }
      : null;

  const pigeonHasTraits = rawPigeonPayload
    ? Object.values(rawPigeonPayload).some(
  ...
  return await prisma.auction.create({
    data: {
      ...
      startingPrice: startingPrice
        ? new Prisma.Decimal(startingPrice)
        : undefined,
      currentPrice: startingPrice
        ? new Prisma.Decimal(startingPrice)
        : buyNowPrice
          ? new Prisma.Decimal(buyNowPrice)
          : 0,
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!userId)
          return res
            .status(401)
            .json({ error: "Musisz być zalogowany, aby wykonać tę akcję." });
        const role = req.user?.role;
        if (role !== "USER_FULL_VERIFIED" && role !== "ADMIN") {
          return res
            .status(403)
            .json({ error: "Twoje konto nie jest w pełni zweryfikowane." });
        }
        if (!prisma)
          return res
            .status(500)
...
router.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      const idValidation = auctionIdSchema.safeParse(req.params.id);
      if (!idValidation.success)
        return res.status(400).json({ error: "Nieprawidłowe ID aukcji." });
      ...
      if (!userId)
        return res
          .status(401)
          .json({ error: "Musisz być zalogowany, aby licytować." });
      const role = req.user?.role;
      if (role !== "USER_FULL_VERIFIED" && role !== "ADMIN") {
        return res
          .status(403)
      ...
      if (message.includes("Auction not active"))
        message = "Aukcja nie jest aktywna.";
      if (message.includes("Outbid"))
        message = "Twoja oferta została przebita.";
      if (message.includes("Bid amount too low"))
        message = "Kwota oferty jest za niska.";
      if (message.includes("Cannot bid on own auction"))
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (!userId)
        return res
          .status(401)
          .json({ error: "Musisz być zalogowany, aby kupić." });
      const role = req.user?.role;
      if (role !== "USER_FULL_VERIFIED" && role !== "ADMIN") {
        return res
          .status(403)
...
router.put(
  "/:id/admin",
  authMiddleware,
  validate(adminUpdateAuctionSchema),
  async (req: AuthenticatedRequest, res) => {
    try {
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const cancelledAuction = await auctionService.adminCancelAuction(
        req.params.id,
      );
